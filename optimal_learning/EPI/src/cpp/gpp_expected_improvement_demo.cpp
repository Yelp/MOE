...
/*
  ...
  4a) Use a randomly constructed Gaussian Process to generate imaginary objective function values
  or
  4b) Use user-provided input training data.*
     * By defining OL_USER_INPUTS to 1, you can specify your own input data.
  5) Select desired concurrent experiment locations (points_to_sample)
...
int main() {
  ...
  // e.g., num_to_sample = 0 means EI optimization will produce the single point
  // that maximizes EI
  // with num_to_sample = 2 (for example), EI optimization will assume that the
  // specified 2 points are \emph{already} being sampled, yielding the next best point
  // outside of these
  // Note: if this value is 0, then we reach a special case of EI that can be computed
  // analytically
  ...
  // points_to_sample, num_to_sample*dim values: the locations of points being concurrently sampled
  ...
  // producing result Xs_0.  Then you rerun EI with 1 points_to_sample = Xs_0, producing Xs_1.
  // Then you run again with 2 points_to_sample = {Xs_0, Xs_1}.
  ...
  // note: this quantity is NOT used if num_to_sample = 0.  In that case, an anlytic
  // formula is used to compute EI, obviating the need for monte-carlo.

  // WARNING: in the analytic case (to_sample = \emptyset) and even using 1000000 mc iterations, we see only about
  ...
  // Still, without changing any other setup, try setting num_to_sample = 0; even with so few MC iterations and the added
  // complexity of points_to_sample, the results are (usually) similar.
  ...
  ComputeOptimalPointToSampleWithRandomStarts(gp, gd_params, domain, points_to_sample.data(), num_to_sample, best_so_far, max_int_steps, max_num_threads, &found_flag, &uniform_generator, normal_rng_vec.data(), next_point_winner.data());
