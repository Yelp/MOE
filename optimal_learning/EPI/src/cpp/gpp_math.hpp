...
/*
  ...
  1) OVERVIEW OF GAUSSIAN PROCESSES AND EXPECTED IMPROVEMENT; WHAT ARE WE TRYING TO DO?
  At a high level, this file optimizes an objective function f(x).  This operation
  ...
  other.  The points x represent experiments. If f(x) is say, survival rate for
  ...
  cannot work with f(x) directly; instead a surrogate is built via interpolation
  ...
  Hence a GP is fully specified by its mean function, m(x), and covariance function,
  k(x,x').  Then we assume that a real process f(x) (e.g., drug survival rate) is
  distributed like:
    f(x) ~ GP(m(x), k(x,x'))
  with m(x) = E[f(x)], k(x,x') = E[(f(x) - m(x))*(f(x') - m(x'))].
  Then sampling from f(x) is simply drawing from a Gaussian with the appropriate mean
  and variance.

  However, since we do not know f(x), we cannot precisely build its corresponding GP.
  Instead, using samples from f(x) (e.g., by measuring experimental outcomes), we can
  iteratively improve our estimate of f(x).  See GaussianProcess class docs
...
/*
  ...
  GPPs estimate a real process f(x) = GP(m(x), k(x,x')) (see file docs).  This class deals with building an estimator
  ...
  NOTE: functions that manipulate the PRNG directly or indirectly (changing state, generating points)
...
struct PointsToSampleState final {
  /*
    ...
    WARNING: This object's state is INVALIDATED if the gaussian_process used in construction is mutated!
    SetupState() should be called again in such a situation.

    WARNING: Using this object to compute gradients when configure_for_gradients := false results in UNDEFINED BEHAVIOR.
  ...
  */
  ...
  /*
    ...
    WARNING: This object's state is INVALIDATED if the gaussian_process used in SetupState is mutated!
    SetupState() should be called again in such a situation.
...
struct ExpectedImprovementState final {
  ...
  */
  ...
  /*
    ...
    WARNING: This object's state is INVALIDATED if the ei_evaluator (including the GaussianProcess it depends on) used in SetupState is mutated!
    SetupState() should be called again in such a situation.
...
struct OnePotentialSampleExpectedImprovementState final {
  ...
  /*
    ...
    Note: num_to_sample = 1 by definition for this case (hence the sizing on grad_mu and grad_chol)
...
/*
  ...
  *WARNING: this function fails ungracefully if NO improvement can be found!  In that case,
...
/*
  ...
  In INPUTS, note the difference between points_to_sample/num_to_sample and num_samples_to_generate.
