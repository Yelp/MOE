...
/*
  ...
  will work), which is in turn the mersenne twister using a common set of parameters.  (The same functionality is also available
  through C++11's <random>.)  Unlike NormalRNG, this class is not a functor since the range and even type can change frequently
  in usage.  Thus the actual distribution should be constructed as needed (e.g., see ComputeLatinHypercubePointsInDomain).
...
struct UniformRandomGenerator final {
  ...
  */
  ...
  /*
    ...
    (see gpp_math) with the "same" randomness.
