...
import moe.optimal_learning.python.lib.mvncdf # multivariate normal cdf
import moe.optimal_learning.python.models.covariance_of_process
...
class OptimalGaussianProcess(GaussianProcess):
    ...
    def get_1D_analytic_expected_improvement(self, point_to_sample):
        ...
        if self.domain and moe.optimal_learning.python.lib.math.not_in_domain(point_to_sample, self.domain):
            return 0.0
    ...
    def get_2D_analytic_expected_improvement(self, point_one, point_two):
        ...
        if self.domain:
            if moe.optimal_learning.python.lib.math.not_in_domain(point_one, self.domain) or moe.optimal_learning.python.lib.math.not_in_domain(point_two, self.domain):
                return 0.0
        ...
        def D_func(m1, m2, s1, s2, c12, best):
            ...
            return moe.optimal_learning.python.lib.mvncdf.mvnormcdf(numpy.array([-numpy.inf,-numpy.inf]), numpy.array([best - m1, m1 - m2]), numpy.array([0.0,0.0]), Gamma)
    ...
    def get_expected_improvement(self, points_to_sample, iterations=1000):
        ...
        if self.domain:
            for point in points_to_sample:
                if moe.optimal_learning.python.lib.math.not_in_domain(point, self.domain):
                    return 0.0
            ...
            if len(improvement) > 1:
                for i in range(len(improvement)):
                    if improvement[i] >= numpy.max(improvement) and improvement[i] > 0.0:
                        ...
                        aggregate_dx -= moe.optimal_learning.python.lib.math.matrix_vector_multiply(grad_chol_decomp, normals[it][:].T)[i]
            else:
                if improvement > 0.0:
    ...
    def get_next_step_multistart(self, starting_points, points_being_sampled, gamma=0.9, iterations=1000, max_N=100):
        ...
        for starting_point in starting_points:
            ...
            if not self.domain or not moe.optimal_learning.python.lib.math.not_in_domain(x_hat, self.domain):
    ...
    def get_multistart_best(self, random_restarts=5, points_being_sampled=[]):
        best_improvement = -numpy.inf
        best_next_step = moe.optimal_learning.python.lib.math.make_rand_point(self.domain)
        for _ in range(random_restarts):
            path, next_step, improvement = self.get_next_step(moe.optimal_learning.python.lib.math.make_rand_point(self.domain), points_being_sampled)
            #path, next_step, improvement = self.get_next_step(numpy.array([-2.141592, 11.274999]), [numpy.array([1.0,0.0])], [0,1])
            if improvement > best_improvement and not moe.optimal_learning.python.lib.math.not_in_domain(next_step, self.domain):
