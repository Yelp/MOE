...
GaussianProcess * make_gaussian_process_new(const boost::python::list& hyperparameters,
                                            const boost::python::list& points_sampled,
                                            const boost::python::list& points_sampled_value,
                                            const boost::python::list& noise_variance,
                                            int dim, int num_sampled, int num_IS) {
  const int num_to_sample = 0;
  const boost::python::list points_to_sample_dummy;
  PythonInterfaceInputContainer input_container(hyperparameters, points_sampled,
                                                points_sampled_value, noise_variance,
                                                points_to_sample_dummy, dim, num_sampled, num_to_sample, num_IS);

  MixedSquareExponential mixed_square_exponential(input_container.dim, num_IS,
                                                  input_container.hyperparameters);

  GaussianProcess * new_gp = new GaussianProcess(mixed_square_exponential, input_container.points_sampled.data(),
                                                 input_container.points_sampled_value.data(),
                                                 input_container.noise_variance.data(),
                                                 input_container.dim, input_container.num_sampled);
  new_gp->SetRandomizedSeed(0);
...
void ExportGaussianProcessFunctions() {
  ...
  boost::python::class_<GaussianProcess, boost::noncopyable>("GaussianProcessNew", boost::python::no_init)
      .def("__init__", boost::python::make_constructor(&make_gaussian_process_new), R"%%(
    Constructor for a ``GPP.GaussianProcess`` object.
