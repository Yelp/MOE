...
/*
  ...
  2) Matrices are stored as flattened arrays and indexed in COLUMN-MAJOR order.
   ...
   4) Keywords:
         ...
         e) we also use several gcc attributes:
            ...
            <> nonnull_pointers: compiler warnings when pointer args are NULL (see details for special case with class member fcns)
            <> warn_unused_result: compiler warning when return value is unused
            <> pure, const: describes a function's dependencies/effects on global state
   ...
   8) Matrix-loop Idiom
      ...
      Pros:
      1) Avoids extra integer arithmetic: redundant i*n computations removed. (Compiler may save off i*n)
         Also greatly reduces number of characters: indexing A[i*dim_1*dim_2 + j*dim_2 + k] for example is unwieldy.
      2) Simple offsets like A[j] can be computed with just one x86 instruction, LEA.  (A[i*n + j] requires 2-3)
         LEA cannot handle A[g + j], so compilers (currently) will not optimize this for you.
      3) This style makes it more clear that the inner loop is just a dot-product.  This may also improve the compiler's
         ability to vectorize.
      Cons:
      1) Overwrites the pointer A.  If the full A matrix is required again, we need to store it before this loop
         or set A -= m*n; after the loop.  (Compiler optimization makes this a non-issue.)
      2) Following 1), leaves the pointer A in an invalid state.  After the loop completes, A[0] would generally
         dereference unallocated space.
      3) Is not the most common idiom and may feel unfamiliar to some programmers.
  ...
  1) comments go above the function declaration or definition they describe
  2) declaration comments should individually describe all inputs/ouputs/returns; e.g.,
     INPUTS:
...
/*
  gcc macro to specify (= promise the compiler) that no NULL pointers will be passed as function args.
