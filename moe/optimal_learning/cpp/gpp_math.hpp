...
class GaussianProcess final {
 ...
 private:
  ...
  void RecomputeDerivedVariables() {
    ...
    if (unlikely(ComputeCholeskyFactorL(num_sampled_, K_chol_.data()) != 0)) {
      OL_THROW_EXCEPTION(SingularMatrixException, "Covariance matrix (K) singular. Check for duplicate points_sampled (with 0 noise) and/or extreme hyperparameter values.", K_chol_.data(), num_sampled_);
    }
