...
class PointGenerationTest(T.TestCase):
    """Test optimal_learning.python.lib.math.get_latin_hypercube_points(num_points, domains)
    also tests optimal_learning.python.lib.math.not_in_domain(point, domain)
    ...
    def test_latin_hypercube_within_domain(self):
        ...
        for domain in self.domains_to_test:
            for num_points in self.num_points_to_test:
                points = moe.optimal_learning.python.lib.math.get_latin_hypercube_points(num_points, domain)

                for point in points:
                    T.assert_equal(self._point_in_domain(point, domain), True)
                    T.assert_equal(moe.optimal_learning.python.lib.math.not_in_domain(point, domain), False)

    def test_make_rand_point_within_domain(self):
        """Tests that make_rand_point(domain) returns a point in the domain
        """
        for domain in self.domains_to_test:
            point = moe.optimal_learning.python.lib.math.make_rand_point(domain)
...
class MatrixOpsTest(T.TestCase):
    """Test the various matrix ops in optimal_learning.python.lib.math
    ...
    def test_cholesky_decomp(self):
        ...
        for size_of_matrix in range(2,10):
            ...
            calculated_cholesky = moe.optimal_learning.python.lib.math.cholesky_decomp(matrix)
    ...
    def test_inverse_via_cholesky(self):
        ...
        for size_of_matrix in range(2,10):
            ...
            A_inv = moe.optimal_learning.python.lib.math.inverse_via_cholesky(A)
            self._assert_matrix_equal(
                    numpy.dot(A, A_inv),
    ...
    def test_inverse_via_backwards_sub(self):
        ...
        for size_of_matrix in range(2,10):
            ...
            A_inv = moe.optimal_learning.python.lib.math.inverse_via_backwards_sub(A)
            self._assert_matrix_equal(
                    numpy.dot(A, A_inv),
                    numpy.diag(numpy.ones(size_of_matrix))
                    )
    ...
    def test_vector_diag_vector_product(self):
        ...
        for size in sizes_to_test:
            ...
            method_result = moe.optimal_learning.python.lib.math.vector_diag_vector_product(
                    rand_vec_one,
                    rand_diag_vec,
                    rand_vec_two
                    )
